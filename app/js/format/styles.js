...
function renderStyleDropdown() {
    ...
    Object.entries(all).forEach(([key, def]) => {
        ...
        // Custom-style labels/keys/preview values can arrive from the opened
        // document — escape all three before they reach this innerHTML.
        const esc = window.escapeHtml || (s => String(s));
        const safeKey = esc(key);

        item.innerHTML = `
            <div class="sdm-preview" style="${esc(previewStyle)}">${esc(def.label)}</div>
            <div class="sdm-actions">
                <button class="sdm-edit-btn" data-key="${safeKey}" title="Stijl bewerken">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                ${!isBuiltin ? `<button class="sdm-del-btn" data-key="${safeKey}" title="Stijl verwijderen"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg></button>` : ''}
            </div>
        `;
